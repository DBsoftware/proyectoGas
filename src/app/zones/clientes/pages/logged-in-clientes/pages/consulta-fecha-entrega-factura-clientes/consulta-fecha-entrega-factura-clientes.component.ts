...
@Component({
  selector: 'app-consulta-fecha-entrega-factura-clientes',
  templateUrl: './consulta-fecha-entrega-factura-clientes.component.html',
  styleUrls: ['./consulta-fecha-entrega-factura-clientes.component.scss']
})
export class ConsultaFechaEntregaFacturaClientesComponent implements OnInit {
  ...
  ngOnInit() {
    ...
    this.filteredMunicipio$ = this.form2.get('municipio').valueChanges
      .pipe(
        ...
        switchMap(value => this.filter(value, `${ AppConfig.API_DOMAIN }:${ AppConfig.PORT }${ AppConfig.API_PREFIX }${ AppConfig.GLOBAL_PREFIX }company/${ AppConfig.COMPANY_ID }/zone/${ ClientesConfig.ZONE_ID }/project/searchbydescription`, AppConfig.yParam))
      );
